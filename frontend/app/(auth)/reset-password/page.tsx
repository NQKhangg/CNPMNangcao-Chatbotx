...
export default function ResetPasswordPage() {
  ...
  const {
    register,
    handleSubmit,
    setValue, // Dùng để set giá trị mặc định cho input
    formState: { errors },
  } = useForm<ResetPasswordForm>();

  // --- 3. EFFECTS ---
  // Tự động điền token nếu trên URL có dạng: /reset-password?token=XYZ
  useEffect(() => {
  ...
  const onSubmit: SubmitHandler<ResetPasswordForm> = async (data) => {
    ...
    try {
      ...
      // Thành công -> Thông báo & Chuyển hướng
  ...
  return (
    <div className="min-h-screen flex items-center justify-center bg-gray-50 px-4 animate-fade-up">
      <div className="max-w-md w-full bg-white rounded-2xl shadow-xl p-8 relative">
        ...
        <Link
          href="/login"
          className="absolute top-8 left-8 text-gray-400 hover:text-slate-900 transition-colors"
